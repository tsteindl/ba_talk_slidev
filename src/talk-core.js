...
const names={brute:'BF (baseline)',linear:'LS',binary_deep:'BS',reverse_eng_risk:'RE',oracle_hl:'Oracle (N_opt)'};
...
function budgetsPlot() {
  ...
  ['brute','linear','binary_deep','reverse_eng_risk'].forEach((key,i)=>{const r=D.crossings.find(r=>r.algorithm===key),y=45+i*65,x=170+.015*+r.budget_to_reach;
    b+=text(145,y+8,names[key],ink,27,'end')+line(170,y,x,y,colors[key],false,13)+line(170+.015*+r.budget_to_reach_lo,y,170+.015*+r.budget_to_reach_hi,y,ink,false,3)+text(x+20,y+8,(+r.budget_to_reach/1000).toFixed(1)+'k',colors[key],28)+(key==='brute'?'':text(x+118,y+8,'('+(+r.budget_to_reach/base).toFixed(2)+')',gray,22));
...
add('Walking toward an invisible boundary','MODEL',states([estimatePlot(D.overshoot,{overshoot:true,count:19}),estimatePlot(D.overshoot,{overshoot:true,count:31}),estimatePlot(D.overshoot,{overshoot:true})])+frag(`<p class="center lead">why? ${m('\\hat\\phi_{\\mathrm{ent}}=\\tfrac1N\\arccos\\sqrt{\\hat p_0}\\leq\\tfrac{\\pi}{2N}\\xrightarrow[N\\to\\infty]{}0')}</p>`,2),
...
add('Back to the original game','RESULTS',resultsPlot()+`<p class="center small accent">all three adaptive strategies improve over the fixed safe baseline</p>`,
  '',55,{estimator:true,scenario:true,source:'50,000 held-out runs/point · 95% Wilson intervals · ◇ analytic asymptotic oracle'});
add('90% successful runs — how much budget?','RESULTS',budgetsPlot()+frag(`<p class="center small muted">bracketed: budget as a share of the brute-force baseline</p>`,0),
  '',40,{estimator:true,scenario:true});
// SKIPPABLE IF RUNNING LONG
add('Tighter precision amortizes exploration','RESULTS',`<img class="verify-figure" style="max-height:430px" src="/assets/thesis/fig_precision.png" alt="Budget advantage over brute force for each adaptive strategy as the precision requirement tightens, with the analytic oracle">`+`<ul class="beat-list" style="max-width:1010px"><li>up to ${m('\\approx1.84\\times')} lower budget in the tested settings*</li><li>coming within ${m('\\approx2\\%')} of the oracle at ${m('\\epsilon=10^{-8}')} (${m('\\approx82\\%')} of its advantage at ${m('\\epsilon=10^{-3}')})</li></ul>`,
  '',35,{estimator:true,scenario:true,className:'precision-slide'});
add('Beyond a branch-inverted point estimate','RESULTS',`<div class="two outlook"><div><h3>BAYESIAN / FULL LIKELIHOOD</h3>${response(50,true)}<ul class="outlook-list"><li>a probabilistic view lets us <b>update a belief</b> instead of inverting once</li><li>current setting: an overshot probe has to be <b>discarded</b></li><li>in a Bayesian setting it can be <b>re-used</b> through the likelihood</li></ul></div><div><h3>TOWARD EXPERIMENT</h3><div style="height:210px;display:flex;align-items:center;justify-content:center;font-size:32px;color:var(--muted)">noise · decoherence · realistic costs</div><ul class="outlook-list"><li>a GHZ state is <b>hard to prepare</b> in the lab — NOON states are the usual stand-in</li><li>test the adaptive protocols on a <b>separable backbone</b> first and compare</li></ul></div></div>`,
  '',45);
add('Summary','RESULTS',`<ul class="summary-list"><li>adaptive algorithms on top of the maximally entangled GHZ protocol<ul><li>Brute Force — always ${m('N_{\\min}')}, no exploration</li><li>Linear Search — walk up until the running mean falls</li><li>Binary Search — bisect, classifying each probe on its own</li><li>Reverse Engineering — invert one safe pilot</li></ul></li><li>results: reverse engineering reaches ${m('90\\%')} convergence on ${m('0.65\\times')} the brute-force budget — ${m('\\approx82\\%')} of the omniscient oracle’s advantage<ul><li>tighter precision widens it to ${m('\\approx1.82\\times')}, within ${m('\\approx2\\%')} of the oracle</li></ul></li></ul><p class="thanks">Thank you for your attention!</p>`,
  '',35);
