...
<template>
  <div class="estimator-story" :data-step="clicks">
    <div class="story-node phase" :class="{ active: clicks === 0 }"><MathEq tex="\phi" :display="false" /><span>hidden phase</span></div>
    <div class="story-arrow"><i :class="{ flow: clicks >= 1 }" />→</div>
    <div class="story-node experiment" :class="{ active: clicks === 1 }">
      <div class="mini-circuit"><b><MathEq tex="|0\rangle" :display="false" /></b><em>H</em><em><MathEq tex="U(\phi)" :display="false" /></em><em>H</em><b>measure</b></div><span>known experiment</span>
    </div>
    ...
    <div class="story-node estimate" :class="{ active: clicks === 3 }"><MathEq tex="\hat p_0=0.60" :display="false" /><span>response estimate</span></div>
    <div class="story-arrow"><i :class="{ flow: clicks >= 4 }" />→</div>
    <div class="story-node answer" :class="{ active: clicks >= 4 }"><MathEq tex="\hat\phi" :display="false" /><span>invert the response</span></div>
  </div>
